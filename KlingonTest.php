...
class KlingonTest
{
    ...
    private function countPrepositions(Collection $words): int
    {
        ...
        return $prepositions->count();
    }

    private function countVerbs(Collection $words, bool $filterFirstPerson = false): int
    {
        $verbs = $words->filter(function ($word) {
            if (strlen($word) < 8) {
                return false;
            }
        ...
        if ($filterFirstPerson) {
            $verbs = $verbs->filter(function ($verb) {
                foreach ($this->fooType as $letter) {
                    if (str_starts_with($verb, $letter)) {
                        return false;
                    }
                }
    ...
    private function getKlingonVocabulary(Collection $words): string
    {
        $vocabulary = [];

        foreach ($this->klingonAlphabet as $letter) {
            $words->filter(function ($word) use ($letter, &$vocabulary) {
                if (str_starts_with($word, $letter)) {
                    array_push($vocabulary, $word);
                }
            });
        }

        return implode(' ', $vocabulary);
    }
}
